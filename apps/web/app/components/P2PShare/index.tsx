...
import { getAvatarSeed, getAvatarInfo } from './utils/avatar'
...
function P2PShareInner({ roomId, roomRole }: P2PShareInnerProps) {
  ...
  useEffect(() => {
    if (
      ...
      toast.dismiss('client-connecting')
    }
  }, [roomRole, connectionStatus])
              ...
              <h3 className='text-xs md:text-sm font-semibold tracking-wider uppercase text-slate-500 font-title border-b border-slate-200 pb-1.5 mb-2.5 md:pb-3 md:mb-4 flex flex-row justify-between items-center gap-2'>
                <div className='flex items-center gap-2 w-full sm:w-auto'>
                  ...
                  <div className='flex items-center gap-1.5 bg-slate-100 pl-1 pr-2.5 h-8 rounded-full border border-slate-200 text-slate-600 max-w-[130px] sm:max-w-none select-none'>
                    ...
                    <svg
                      className='w-4 h-4 text-slate-500 ml-0.5'
                      fill='none'
                      stroke='currentColor'
                      viewBox='0 0 24 24'
                    >
